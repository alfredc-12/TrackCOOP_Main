...
Membership application uploads use the protected storage path
`storage/protected/membership-applications`. The API validates request shape,
file extension/MIME type, basic file signatures where applicable, file size, and
SHA-256 checksum metadata. Stored filenames are generated by the server and
clients receive document metadata, not filesystem paths.

The older communication image-upload path still serves selected public landing
assets from `storage/public/uploads` through `/uploads`. Use it only for public
website media, not membership documents, payment proofs, IDs, or private member
files.
...
- Public membership tracking and activation URLs contain one-time raw secrets.
  They must be delivered to the intended applicant only and must not be pasted
  into logs, screenshots, seed files, or committed documentation.
