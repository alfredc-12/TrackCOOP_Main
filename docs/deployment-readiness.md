...
| `PAYMONGO_TEST_PAYMENT_METHOD_TYPES` | No | Test mode only | No | TrackCOOP expects `qrph`. |
...
| `PAYMONGO_LIVE_PAYMENT_METHOD_TYPES` | No | Live mode only | No | TrackCOOP expects `qrph`. |
...
TrackCOOP's supported PayMongo setup uses Hosted Checkout with QR Ph in both
modes.
...
Normal local QR Ph test mode should keep:
...
The public success-page status poll uses a signed opaque `statusToken` generated
into the PayMongo success URL. Public status polling does not expose PayMongo
checkout, payment, or payment-intent IDs. Authenticated staff/member status
routes remain available for internal views.

The success page polls TrackCOOP every 2.5 seconds for up to 120 checks, or
about 5 minutes. Polling stops earlier when the database becomes `Validated`, a
terminal failed/reversed status is returned, or the timeout is reached. The
redirect itself never marks a payment as paid.

For QR Ph in PayMongo test mode, do not send real money to the displayed test
QR. Use PayMongo's official test/simulation flow for the test checkout session,
such as the Hosted Checkout `test_url` when PayMongo provides it for the test
resource, then let the signed test webhook update TrackCOOP.
