...
export const createAnnouncementSchema = z.object({
  ...
  audienceTargets: z.array(z.string().trim().min(1).max(190)).max(100).optional(),
  audienceValues: z.array(z.string().trim().min(1).max(190)).max(100).optional(),
  announcementStatus: z.enum(announcementStatuses).default("Draft"),
  featuredImagePath: z.string().trim().max(500).nullable().optional(),
  images: z.array(z.string().trim().max(500)).max(12).optional(),
