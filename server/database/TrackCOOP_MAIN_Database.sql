...
CREATE INDEX `idx_user_sessions_user_active` ON `user_sessions` (user_id, expires_at, revoked_at);
...
CREATE TABLE user_activation_tokens (
    ...
    CONSTRAINT fk_user_activation_token_user FOREIGN KEY (user_id) REFERENCES users(user_id)
        ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_user_activation_token_creator FOREIGN KEY (created_by) REFERENCES users(user_id)
        ON UPDATE CASCADE ON DELETE SET NULL) ENGINE=InnoDB;
...
CREATE INDEX `idx_members_type_status` ON `member_profiles` (membership_type, approval_status, official_member_status);
...
CREATE TABLE membership_applications (
    ...
    application_status ENUM('Submitted', 'Under Review', 'Needs Information', 'Approved', 'Rejected', 'Withdrawn') NOT NULL DEFAULT 'Submitted',
    ...
    CONSTRAINT fk_membership_application_submitter FOREIGN KEY (submitted_by_user_id) REFERENCES users(user_id)
        ON UPDATE CASCADE ON DELETE SET NULL,
    CONSTRAINT fk_membership_application_reviewer FOREIGN KEY (reviewed_by) REFERENCES users(user_id)
        ON UPDATE CASCADE ON DELETE SET NULL,
    CONSTRAINT fk_membership_application_converted_member FOREIGN KEY (converted_member_id) REFERENCES member_profiles(member_id)
        ON UPDATE CASCADE ON DELETE SET NULL) ENGINE=InnoDB;
...
CREATE TABLE membership_application_beneficiaries (
    ...
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT fk_membership_beneficiary_application FOREIGN KEY (membership_application_id) REFERENCES membership_applications(membership_application_id)
        ON UPDATE CASCADE ON DELETE CASCADE) ENGINE=InnoDB;
...
CREATE TABLE membership_application_documents (
    ...
    CONSTRAINT fk_membership_document_application FOREIGN KEY (membership_application_id) REFERENCES membership_applications(membership_application_id)
        ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_membership_document_uploader FOREIGN KEY (uploaded_by_user_id) REFERENCES users(user_id)
        ON UPDATE CASCADE ON DELETE SET NULL) ENGINE=InnoDB;
...
CREATE TABLE membership_application_status_history (
    ...
    membership_application_id BIGINT UNSIGNED NOT NULL,
    old_status ENUM('Submitted', 'Under Review', 'Needs Information', 'Approved', 'Rejected', 'Withdrawn') NULL,
    new_status ENUM('Submitted', 'Under Review', 'Needs Information', 'Approved', 'Rejected', 'Withdrawn') NOT NULL,
    internal_note TEXT NULL,
    applicant_message TEXT NULL,
    changed_by BIGINT UNSIGNED NULL,
    changed_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_membership_application_history_application FOREIGN KEY (membership_application_id) REFERENCES membership_applications(membership_application_id)
        ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_membership_application_history_user FOREIGN KEY (changed_by) REFERENCES users(user_id)
        ON UPDATE CASCADE ON DELETE SET NULL) ENGINE=InnoDB;
...
CREATE INDEX `idx_payment_reference_status` ON `payment_references` (validation_status, payment_purpose, submitted_at);
CREATE INDEX `idx_payment_reference_related` ON `payment_references` (related_entity_type, related_entity_id);
...
CREATE TABLE membership_application_requirements (
    ...
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT fk_membership_requirement_application FOREIGN KEY (membership_application_id) REFERENCES membership_applications(membership_application_id)
        ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_membership_requirement_payment FOREIGN KEY (payment_reference_id) REFERENCES payment_references(payment_reference_id)
        ON UPDATE CASCADE ON DELETE SET NULL,
    CONSTRAINT fk_membership_requirement_document FOREIGN KEY (membership_application_document_id) REFERENCES membership_application_documents(membership_application_document_id)
        ON UPDATE CASCADE ON DELETE SET NULL,
    CONSTRAINT fk_membership_requirement_verifier FOREIGN KEY (verified_by) REFERENCES users(user_id)
        ON UPDATE CASCADE ON DELETE SET NULL) ENGINE=InnoDB;
...
CREATE INDEX `idx_share_payment_member_status` ON `share_capital_payments` (member_id, payment_status, payment_date);
...
CREATE INDEX `idx_financial_record_date_type` ON `financial_records` (record_date, record_type, record_status);
CREATE INDEX `idx_financial_category_date` ON `financial_records` (financial_category_id, record_date);
CREATE INDEX `idx_financial_source` ON `financial_records` (source_module, source_record_id);
...
CREATE INDEX `idx_pos_sales_date_status` ON `pos_sales` (sale_date, sale_status, payment_status);
...
CREATE INDEX `idx_rental_bookings_asset_schedule` ON `rental_bookings` (rental_asset_id, start_datetime, end_datetime, booking_status);
CREATE INDEX `idx_rental_bookings_member` ON `rental_bookings` (member_id, created_at);
CREATE INDEX `idx_rental_bookings_status` ON `rental_bookings` (booking_status, payment_status, start_datetime);
...
CREATE INDEX `idx_rental_pos_date_type` ON `rental_pos_records` (transaction_date, transaction_type);
...
CREATE INDEX `idx_documents_access_type` ON `documents` (access_level, document_type, document_status);
...
CREATE INDEX `idx_announcements_publication` ON `announcements` (announcement_status, audience_type, publish_at);
...
CREATE INDEX `idx_requests_status_priority` ON `requests_inquiries` (request_status, priority, submitted_at);
...
CREATE INDEX `idx_site_content_page_status` ON `site_content_blocks` (page_slug, content_status, display_order);
...
CREATE INDEX `idx_services_public` ON `services` (service_status, public_visibility, display_order);
...
CREATE INDEX `idx_programs_projects_status` ON `programs_projects` (status, public_visibility, display_order);
...
CREATE INDEX `idx_partners_certifications_status` ON `partners_certifications` (record_type, status, public_visibility, display_order);
...
CREATE INDEX `idx_gallery_public` ON `gallery_items` (gallery_status, public_visibility, activity_date, display_order);
-- ============================================================================
...
CREATE INDEX `idx_audit_logs_entity` ON `audit_logs` (entity_table, record_id, action_time);
...
VALUES
    ('Chairman/Admin', 'chairman_admin', 'Administrative, monitoring, configuration, approval, reporting, and audit access.', 1),
    ('Bookkeeper', 'bookkeeper', 'Financial recording, payment validation, share capital, rental/POS, receipts, and financial reports.', 1),
    ('Member', 'member', 'Member self-service access to own profile, payments, share capital, documents, announcements, and requests.', 1)
...
(category_code, category_name, category_type, description, is_system_category, is_active)
VALUES
    ('MEMBERSHIP_FEE', 'Membership Fees', 'Income', 'Associate membership and other approved membership fees.', 1, 1),
    ('RENTAL_INCOME', 'Rental Income', 'Income', 'Income from equipment, service, or facility rental.', 1, 1),
    ('POS_SALES', 'POS and Product Sales', 'Income', 'Income from product and POS transactions.', 1, 1),
    ('PROGRAM_INCOME', 'Program or Project Income', 'Income', 'Income from cooperative programs and approved projects.', 1, 1),
    ('DONATIONS_GRANTS', 'Donations and Grants', 'Income', 'Approved donations, grants, and assistance.', 1, 1),
    ('OTHER_INCOME', 'Other Income', 'Income', 'Other approved cooperative income.', 1, 1),
    ('PRODUCT_COST', 'Product Cost', 'Expense', 'Cost related to products sold through POS.', 1, 1),
    ('FUEL_GASOLINE', 'Fuel and Gasoline', 'Expense', 'Fuel or gasoline expense subject to approved rental rules.', 1, 1),
    ('REPAIR_MAINTENANCE', 'Repair and Maintenance', 'Expense', 'Repair and maintenance of equipment or facilities.', 1, 1),
    ('OFFICE_SUPPLIES', 'Office Supplies', 'Expense', 'Office and administrative supplies.', 1, 1),
    ('UTILITIES', 'Utilities', 'Expense', 'Electricity, water, internet, and other utility costs.', 1, 1),
    ('TRANSPORTATION', 'Transportation', 'Expense', 'Approved transportation-related costs.', 1, 1),
    ('OTHER_EXPENSE', 'Other Expense', 'Expense', 'Other approved cooperative expenses.', 1, 1)
ON DUPLICATE KEY UPDATE
...
VALUES
    ('Business Rules', 'business.associate_membership_fee', '200.00', 'Number', 'Confirmed associate membership fee in Philippine pesos.', 0, CURRENT_DATE),
    ('Business Rules', 'business.true_member_share_capital_required', '3000.00', 'Number', 'Confirmed share capital required for true membership.', 0, CURRENT_DATE),
    ('Business Rules', 'business.initial_share_capital_payment', '1500.00', 'Number', 'Confirmed initial share capital payment amount.', 0, CURRENT_DATE),
    ('Business Rules', 'business.share_capital_completion_months', '12', 'Number', 'Confirmed completion period in months.', 0, CURRENT_DATE),
    ('Business Rules', 'business.maximum_share_capital', '15000.00', 'Number', 'Confirmed maximum share capital per member.', 0, CURRENT_DATE),
    ('General', 'general.currency_code', 'PHP', 'String', 'System currency code.', 1, CURRENT_DATE),
    ('General', 'general.currency_symbol', '₱', 'String', 'System currency symbol.', 1, CURRENT_DATE),
    ('Inventory', 'inventory.allow_negative_stock', 'false', 'Boolean', 'Prevent negative stock unless formally changed by an authorized administrator.', 0, CURRENT_DATE),
    ('Rental', 'rental.rules_status', 'Pending NFFAC Validation', 'String', 'Rates, discounts, fuel, cancellation, rescheduling, damage, and late-return rules remain configurable.', 0, CURRENT_DATE),
    ('POS', 'pos.rules_status', 'Pending NFFAC Validation', 'String', 'Product approval, inventory deduction, preorder, bulk order, returns, and refund rules remain configurable.', 0, CURRENT_DATE),
    ('Analytics', 'analytics.member_status_labels', '["Active","Needs Monitoring","Inactive"]', 'JSON', 'Approved descriptive member engagement labels.', 0, CURRENT_DATE),
    ('membership', 'membership.associate_fee', '200', 'Number', 'Associate membership fee in Philippine pesos.', 0, CURRENT_DATE),
    ('membership', 'membership.initial_share_capital', '1500', 'Number', 'Initial share-capital payment in Philippine pesos.', 0, CURRENT_DATE),
    ('membership', 'membership.true_member_required_capital', '3000', 'Number', 'Share-capital target required for true-member approval in Philippine pesos.', 0, CURRENT_DATE),
    ('membership', 'membership.maximum_share_capital', '15000', 'Number', 'Maximum validated member share capital in Philippine pesos.', 0, CURRENT_DATE),
    ('membership', 'membership.share_capital_deadline_months', '12', 'Number', 'Number of months allowed to complete the true-member share-capital target.', 0, CURRENT_DATE),
    ('membership', 'membership.orientation_required', 'true', 'Boolean', 'Whether applicant orientation or seminar completion is required before approval.', 0, CURRENT_DATE),
    ('membership', 'membership.activation_token_hours', '72', 'Number', 'Number of hours a member account activation token remains valid.', 0, CURRENT_DATE),
    ('membership', 'membership.terms_version', '2026-07-24', 'String', 'Membership terms and consent version used for new applications.', 0, CURRENT_DATE),
    ('member_indicators', 'member_indicators.minimum_quintile_population', '5', 'Number', 'Minimum member population before indicator scoring uses deterministic quintile ranks.', 0, CURRENT_DATE),
    ('member_indicators', 'member_indicators.fallback_thresholds', '{"recencyDays":[{"max":30,"score":5},{"max":90,"score":4},{"max":180,"score":3},{"max":365,"score":2}],"frequencyCount":[{"min":12,"score":5},{"min":6,"score":4},{"min":3,"score":3},{"min":1,"score":2}],"contributionAmount":[{"min":10000,"score":5},{"min":5000,"score":4},{"min":1500,"score":3},{"min":1,"score":2}]}', 'JSON', 'Fallback 1-5 indicator thresholds used when the member population is too small for stable quintile ranks.', 0, CURRENT_DATE),
    ('member_indicators', 'member_indicators.label_thresholds', '{"activeMin":12,"needsMonitoringMin":7}', 'JSON', 'Total-score thresholds for advisory member indicator labels.', 0, CURRENT_DATE)
ON DUPLICATE KEY UPDATE
...
SELECT
    ...
    COALESCE(SUM(CASE WHEN scp.payment_status = 'Validated' THEN scp.amount ELSE 0 END), 0.00) AS validated_share_capital,
    GREATEST(
        CAST(COALESCE((SELECT setting_value FROM system_settings WHERE setting_key = 'business.true_member_share_capital_required' LIMIT 1), '3000.00') AS DECIMAL(12,2))
        - COALESCE(SUM(CASE WHEN scp.payment_status = 'Validated' THEN scp.amount ELSE 0 END), 0.00),
        0.00
    ) AS remaining_for_true_membership,
    CASE
        WHEN COALESCE(SUM(CASE WHEN scp.payment_status = 'Validated' THEN scp.amount ELSE 0 END), 0.00)
             >= CAST(COALESCE((SELECT setting_value FROM system_settings WHERE setting_key = 'business.initial_share_capital_payment' LIMIT 1), '1500.00') AS DECIMAL(12,2))
        THEN 1 ELSE 0
...
GROUP BY
    m.member_id, m.member_code, m.full_name, m.membership_type,
    m.approval_status, m.official_member_status, m.share_capital_deadline;
...
SELECT
    ...
    p.reorder_level,
    p.product_status,
    COALESCE(SUM(im.quantity_change), 0.000) AS quantity_on_hand,
    CASE
        WHEN p.track_inventory = 0 THEN 'Not Tracked'
        WHEN COALESCE(SUM(im.quantity_change), 0.000) <= 0 THEN 'Out of Stock'
        WHEN COALESCE(SUM(im.quantity_change), 0.000) <= p.reorder_level THEN 'Low Stock'
        ELSE 'In Stock'
    END AS stock_status
FROM products p
LEFT JOIN inventory_movements im ON im.product_id = p.product_id
GROUP BY
    p.product_id, p.sku, p.product_name, p.category, p.unit,
    p.selling_price, p.reorder_level, p.product_status, p.track_inventory;

DROP VIEW IF EXISTS v_latest_member_status_indicator;
CREATE VIEW v_latest_member_status_indicator AS
SELECT msi.*
FROM member_status_indicators msi
INNER JOIN (
    SELECT member_id, MAX(computed_at) AS latest_computed_at
    FROM member_status_indicators
    GROUP BY member_id
) latest
    ON latest.member_id = msi.member_id
...
SELECT
    ...
    SUM(CASE WHEN record_type = 'Income' AND record_status = 'Active' THEN amount ELSE 0 END) AS total_income,
    SUM(CASE WHEN record_type = 'Expense' AND record_status = 'Active' THEN amount ELSE 0 END) AS total_expense,
    SUM(CASE
        WHEN record_type = 'Income' AND record_status = 'Active' THEN amount
        WHEN record_type = 'Expense' AND record_status = 'Active' THEN -amount
        ELSE 0
    END) AS net_movement
FROM financial_records
GROUP BY DATE_FORMAT(record_date, '%Y-%m-01');

DROP VIEW IF EXISTS v_barangay_member_distribution;
CREATE VIEW v_barangay_member_distribution AS
SELECT
    COALESCE(NULLIF(TRIM(barangay), ''), 'Unspecified') AS barangay,
    COUNT(*) AS total_members,
    SUM(CASE WHEN membership_type = 'Associate' THEN 1 ELSE 0 END) AS associate_members,
    SUM(CASE WHEN membership_type = 'True Member' THEN 1 ELSE 0 END) AS true_members,
    SUM(CASE WHEN official_member_status = 'Active' THEN 1 ELSE 0 END) AS active_official_members
FROM member_profiles
WHERE approval_status = 'Approved'
GROUP BY COALESCE(NULLIF(TRIM(barangay), ''), 'Unspecified');

DROP VIEW IF EXISTS v_dashboard_financial_overview;
CREATE VIEW v_dashboard_financial_overview AS
SELECT
    COALESCE(SUM(CASE WHEN record_type = 'Income' AND record_status = 'Active' THEN amount ELSE 0 END), 0.00) AS total_income,
    COALESCE(SUM(CASE WHEN record_type = 'Expense' AND record_status = 'Active' THEN amount ELSE 0 END), 0.00) AS total_expense,
    COALESCE(SUM(CASE
        WHEN record_type = 'Income' AND record_status = 'Active' THEN amount
        WHEN record_type = 'Expense' AND record_status = 'Active' THEN -amount
        ELSE 0
    END), 0.00) AS available_balance,
    (SELECT COALESCE(SUM(amount), 0.00) FROM share_capital_payments WHERE payment_status = 'Validated') AS total_share_capital,
    (SELECT COUNT(*) FROM payment_references WHERE validation_status = 'Pending') AS pending_payment_references,
    (SELECT COUNT(*) FROM rental_bookings WHERE booking_status IN ('Inquiry', 'Pending', 'Approved', 'Scheduled', 'In Use')) AS open_rental_bookings,
    (SELECT COUNT(*) FROM requests_inquiries WHERE request_status NOT IN ('Resolved', 'Closed', 'Rejected', 'Cancelled')) AS open_requests
FROM financial_records;
