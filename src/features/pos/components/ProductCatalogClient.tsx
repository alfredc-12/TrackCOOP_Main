      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="sticky top-0 z-20 mb-6 rounded-2xl border border-[#DCE9DE] bg-white/90 p-4 shadow-sm backdrop-blur-md">
          ...
          <div className="flex items-center gap-2">
            <div className="hidden sm:block"><CatalogThemedSelect value={statusFilter} onChange={(value) => { setStatusFilter(value); setCurrentPage(1); }} ariaLabel="Filter product status" options={[{ value: "All", label: "All status" }, { value: "Available", label: "Available" }, { value: "Unavailable", label: "Unavailable" }]} /></div>
            ...
            <button type="button" onClick={() => void refreshProducts()} disabled={isRefreshing} className="inline-flex items-center gap-2 rounded-xl bg-[#123D2A] px-4 py-3 text-sm font-bold text-white transition hover:bg-[#1F6B43] disabled:opacity-60"><RefreshCw className={`size-4 ${isRefreshing ? "animate-spin" : ""}`} /><span className="hidden sm:inline">Refresh</span></button>
          </div>
          </div>
              ...
              return (
                <article key={item.id} className="group overflow-hidden rounded-2xl border border-[#d8e4d6] bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_30px_rgba(18,61,42,0.12)]">
                  ...
                  <div className="p-5">
                    ...
                    <button type="button" onClick={() => setDetailsProduct(item)} className="mb-4 inline-flex items-center gap-2 text-sm font-bold text-[#1F6B43] transition hover:text-[#123D2A]"><Eye className="size-4" /> View product details</button>
