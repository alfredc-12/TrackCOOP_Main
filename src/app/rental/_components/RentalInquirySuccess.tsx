...
export function RentalInquirySuccess({ inModal, onDismiss }: { inModal?: boolean; onDismiss?: () => void }) {
  ...
  return (
    <div className={`mx-auto max-w-3xl ${inModal ? "" : "px-4 py-12 sm:px-6"}`}>
      <section
        data-rental-print
        data-rental-print-summary
        className={`bg-white text-center ${inModal ? "px-2 py-6" : "rounded-3xl border border-[#cfe0ca] p-6 shadow-xl shadow-[#123d2a]/8 sm:p-10"}`}
      >
        <CheckCircle2 className="mx-auto size-16 text-[#1f6b43]" />
        <p className="mt-5 text-sm font-extrabold uppercase tracking-[0.18em] text-[#497158]">
          Booking submitted successfully
        </p>
        <h1 className="mt-2 text-4xl font-extrabold text-[#123d2a]">
          {inquiry.inquiryId}
        </h1>
        <p className="mx-auto mt-4 max-w-xl leading-7 text-[#5d6d62]">
          NFFAC will review your request and contact you regarding availability,
          schedule, pricing, and rental conditions.
        </p>
        <dl className="mt-8 grid gap-4 rounded-2xl bg-[#f5f8f1] p-5 text-left sm:grid-cols-2">
          <SuccessDetail term="Requester" value={inquiry.requester.fullName} />
          ...
          <div>
            ...
            <dd className="mt-1"><RentalStatusBadge status={inquiry.status} /></dd>
          </div>
        </dl>
        <div data-no-print className="mt-8 grid gap-3 sm:grid-cols-2">
          ...
          {!inModal ? <Link href="/rental" className="inline-flex min-h-11 items-center justify-center rounded-xl px-5 text-sm font-bold text-[#365f4a]">Return to Rental Services</Link> : null}
        </div>
      </section>
    </div>
  );
